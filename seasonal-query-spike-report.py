...
def main():
    ...
    for i in range(args.months):
        ...
        if os.path.exists(cache_file):
            ...
            # Check for existing gsc-pages-queries report in output folder
            site_output_dir = os.path.join('output', host_dir)
            pattern = os.path.join(site_output_dir, f"gsc-pages-queries-*-{start_date}-to-{end_date}.csv")
            matching_files = glob.glob(pattern)
            if matching_files:
                print(f"Found existing query report for {month_str}: {matching_files[0]}")
                df_full = pd.read_csv(matching_files[0])
                # gsc-pages-queries typically has 'query' and 'page'. We need to aggregate by query.
                if 'query' in df_full.columns:
                    df_month = df_full.groupby('query').agg({'clicks': 'sum', 'impressions': 'sum'}).reset_index()
                    # Save to cache
                    df_month[['query', 'clicks', 'impressions']].to_csv(cache_file, index=False)
